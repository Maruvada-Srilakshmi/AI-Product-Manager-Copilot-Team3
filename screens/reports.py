...
def show_reports():
    ...
    if gen_clicked and not features.empty:
        ...
        # PRD -> Roadmap handoff: once a PRD exists for a feature, it's real
        # enough to plan, so schedule it onto the roadmap right away via the
        # Roadmap Planning Agent instead of waiting for a separate manual step.
        ...
        with st.expander("Quick PRD from raw feedback (no ingestion needed)"):
            ...
            if stages:
                ...
                if st.button("Save PRD to documents", key="save_quick_prd"):
                    ...
                    existing_feature = features[features["title"] == title_guess] if not features.empty else features
