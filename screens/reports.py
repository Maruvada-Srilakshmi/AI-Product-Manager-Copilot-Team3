...
def _generate_and_save_user_stories(target):
    """
    Drafts and saves a standalone User Stories document for a single
    feature request -- separate from the "User Stories" section already
    embedded inside a generated PRD. Uses src.llm.generate_user_stories,
    which previously had no caller anywhere in the app. Doesn't schedule
    anything onto the roadmap (that handoff stays specific to the full PRD
    flow above) -- this just generates and saves the document.
    """
    with st.spinner(f"Drafting user stories for '{target.get('description') or target['title']}'..."):
        stories = generate_user_stories(target["title"], target["description"] or "")
    execute(
        "INSERT INTO documents (workspace_id, doc_type, title, content, created_at) VALUES (?,?,?,?,?)",
        (ws_id(), "User Stories", target["title"], stories, now()),
    )
    st.session_state["prd_success"] = (
        f"New user stories generated for '{target.get('description') or target['title']}'!"
    )
...
def show_reports():
    ...
    header_col, btn_col, stories_btn_col = st.columns([4, 1.5, 1.9])
    with header_col:
    ...
    with stories_btn_col:
        st.write("")
        gen_stories_clicked = st.button("+ Generate User Stories", use_container_width=True,
                                         disabled=features.empty)
    ...
    if gen_stories_clicked and not features.empty:
        # Generate for the highest-voted feature request that doesn't have user stories yet, else the top one.
        stories_titles = set(docs[docs["doc_type"] == "User Stories"]["title"]) if not docs.empty else set()
        candidates = features[~features["title"].isin(stories_titles)]
        target = candidates.iloc[0] if not candidates.empty else features.iloc[0]
        _generate_and_save_user_stories(target)
        docs = fetch_documents()
        docs = docs[docs["doc_type"] != "Roadmap"] if not docs.empty else docs
    ...
    with tab_prd:
        ...
        if docs.empty:
            st.info("No documents generated yet. Click **+ Generate New PRD** or **+ Generate User "
                    "Stories** above once you have feature requests.")
    ...
    with tab_summary:
        ...
        c5.metric("User Stories Generated",
                  str(len(docs[docs["doc_type"] == "User Stories"])) if not docs.empty else "0")
